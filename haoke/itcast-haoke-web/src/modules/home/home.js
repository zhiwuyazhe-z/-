...
const client = new ApolloClient({
    uri: "http://192.168.8.128:18080/graphql"
});
...
class Home extends React.Component {
  ...
  // })
    ...
    let swipe = new Promise((resolve, reject) => {
      axios.get('http://192.168.8.128:18080/ad').then((data)=>{
        resolve(data.data.list);
      });
    })

    let menu = new Promise((resolve, reject) => {
        axios.get('http://192.168.8.128:18080/mock/index/menu').then((data)=>{
            resolve(data.data.list);
        });
    })
    let info = new Promise((resolve, reject) => {
      axios.get('http://192.168.8.128:18080/mock/index/info').then((data)=>{
        resolve(data.data.list);
      });
    })
    let faq = new Promise((resolve, reject) => {
      axios.get('http://192.168.8.128:18080/mock/index/faq').then((data)=>{
        resolve(data.data.list);
      });
    })
    let house = new Promise((resolve, reject) => {
      axios.get('http://192.168.8.128:18080/mock/index/house').then((data)=>{
        resolve(data.data.list);
      });
    })
